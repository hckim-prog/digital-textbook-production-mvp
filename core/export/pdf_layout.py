...
class CodeBlock(Flowable):
    def __init__(self, block_id, text, evidence, first_line=0, size=None, kind="code-block"):
        super().__init__()
    ...
    def split(self, available_width, available_height):
        ...
        if count < 1 or count >= len(self.lines):
            return []
        return [CodeBlock(self.block_id, "\n".join(self.lines[:count]), self.evidence, self.first_line, self.size, self.kind),
                CodeBlock(self.block_id, "\n".join(self.lines[count:]), self.evidence, self.first_line + count, self.size, self.kind)]
